...
import { calculatePecentage, getChartData, getInventories, } from "../utils/features.js";
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransactionsPromise = Order.find({})
            ...
            .limit(4);
        const [thisMonthProduct, thisMonthUsers, thisMonthOrders, lastMonthProduct, lastMonthUsers, lastMonthOrders, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUserCount, latestTransactions,] = await Promise.all([
        ...
        const changePercent = {
            revenue: calculatePecentage(thisMonthRevenue, lastMonthRevenue),
            products: calculatePecentage(thisMonthProduct.length, lastMonthProduct.length),
            users: calculatePecentage(thisMonthUsers.length, lastMonthUsers.length),
            orders: calculatePecentage(thisMonthOrders.length, lastMonthOrders.length),
        };
...
export const getLineCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [products, users, orders] = await Promise.all([
            Product.find(baseQuerry).select("createdAt"),
